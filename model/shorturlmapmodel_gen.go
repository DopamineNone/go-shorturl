...
func (m *defaultShortUrlMapModel) Insert(ctx context.Context, data *ShortUrlMap) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.IsDel, data.Lurl, data.Md5, data.Surl)
	return ret, err
}
...
func (m *defaultShortUrlMapModel) Update(ctx context.Context, newData *ShortUrlMap) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.IsDel, newData.Lurl, newData.Md5, newData.Surl, newData.Id)
	return err
}
